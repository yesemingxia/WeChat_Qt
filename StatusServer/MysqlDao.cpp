...
bool MysqlDao::CheckEmail(const std::string& name, const std::string& email) {
    ...
    try {
        ...
        auto result = con->sql("SELECT email FROM user WHERE name = :name")
            .bind("name", name)
            .execute();
...
bool MysqlDao::UpdatePwd(const std::string& name, const std::string& newpwd) {
    ...
    try {
        ...
        auto result = con->sql("UPDATE user SET pwd = :pwd WHERE name = :name")
            .bind("pwd", newpwd)
            .bind("name", name)
            .execute();
...
bool MysqlDao::CheckPwd(const std::string& name, const std::string& pwd, UserInfo& userInfo) {
    ...
    try {
        ...
        // 执行参数化查询
        // 注意选择字段的顺序：uid(0), name(1), pwd(2), email(3)
        auto result = con->sql("SELECT uid, name, pwd, email FROM user WHERE name = :name")
            .bind("name", name)
            .execute();
        ...
        // 3. 处理可能为 NULL 的字段（如 email）
        if (!row[3].isNull()) {
            userInfo.email = (std::string)row[3];
        }
        else {
            userInfo.email = "";
        }
